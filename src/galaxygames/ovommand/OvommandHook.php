...
foreach($commandDataList as $i => $commandData){
    ...
    // 2) reemplazar SOLO overloads, manteniendo lo demás igual
    $commandDataList[$i] = new CommandData(
        ...
        overloads: self::generateOverloads($player, $command),
        chainedSubCommandData: $commandData->getChainedSubCommandData()
    );
}

// 3) re-ensamblar a RAW otra vez
$newPacket = AvailableCommandsPacketAssembler::assemble(
    commandData: $commandDataList,
    hardcodedEnums: [],
    hardcodedSoftEnums: []
);

// 4) copiar los arrays RAW al packet original (mutación in-place)
$packet->enumValues = $newPacket->enumValues;
$packet->chainedSubCommandValues = $newPacket->chainedSubCommandValues;
$packet->postfixes = $newPacket->postfixes;
$packet->enums = $newPacket->enums;
$packet->chainedSubCommandData = $newPacket->chainedSubCommandData;
$packet->commandData = $newPacket->commandData;
$packet->softEnums = $newPacket->softEnums;
$packet->enumConstraints = $newPacket->enumConstraints;
				...
				$commandMap = Server::getInstance()->getCommandMap();
				foreach ($packet->commandData as $i => $commandData) {
					$command = $commandMap->getCommand($commandData->getName());
					if (!$command instanceof Ovommand) {
						continue;
					}
                    ...
                    $packet->commandData[$i] = new CommandData(
                        name: $commandData->getName(),
                        description: $commandData->getDescription(),
                        flags: $commandData->getFlags(),
                        permission: $commandData->getPermission(),
                        aliases: $commandData->getAliases(), // puede ser null o CommandHardEnum
                        overloads: self::generateOverloads($player, $command),
                        chainedSubCommandData: $commandData->getChainedSubCommandData()
                    );
	...
	private static function generateOverloads(CommandSender $sender, Ovommand $command) : array{
		...
		foreach ($command->getSubCommands() as $label => $subCommand) {
			...
			$scParam = CommandParameter::standard($enumName, CommandParameterTypes::RAWTEXT, 0, false);
